...
def select_lines(img):
    ...
    cv2.setMouseCallback("Select Lines", select_line)
...
def draw_boxes(img, bbox, names, object_id, identities=None, offset=(0, 0)):
    ...
    canvas1.itemconfigure(process_started_text, text="Inside draw boxes.....")
...
class DetectionPredictor(BasePredictor):
    ...
    def get_annotator(self, img):
        canvas1.itemconfigure(process_started_text, text="In get annotator.....")
        return Annotator(img, line_width=self.args.line_thickness, example=str(self.model.names))

    def preprocess(self, img):
        canvas1.itemconfigure(process_started_text, text="In pre-porcess.....")
    ...
    def postprocess(self, preds, img, orig_img):
        canvas1.itemconfigure(process_started_text, text="In post process.....")
    ...
    def write_results(self, idx, preds, batch):
        canvas1.itemconfigure(process_started_text, text="In wirte res.....")
        p, im, im0 = batch
        all_outputs = []
        log_string = ""
        if len(im.shape) == 3:
...
def predict(cfg):
    ...
    canvas1.itemconfigure(process_started_text, text="Starting.")
    cfg.imgsz = check_imgsz(cfg.imgsz, min_dim=2)  # check image size
    canvas1.itemconfigure(process_started_text, text="Starting..")
    print("==========\nFile Path", video_path)
    cfg.source = video_path
    canvas1.itemconfigure(process_started_text, text="Starting..."+ cfg.source+" "+ cfg.model)
    print("cfg.source", cfg.source)
    predictor = DetectionPredictor(cfg)
    canvas1.itemconfigure(process_started_text, text="Starting.....")
    predictor()
    canvas1.itemconfigure(process_started_text, text="Started")
...
def upload():
    ...
    canvas1.itemconfigure(file_selected_text, text=os.path.basename(video_path))


# Shared flag variable to indicate if the execution should be interrupted

def process():
    global video_path
    print("=================================\nprocess input", video_path)
    global interrupt_flag

    canvas1.itemconfigure(process_started_text, text="Processing Started. \n   Please wait...")
    # Reset the interrupt flag
    interrupt_flag = False
    # Start a new thread to run the detect.foo() method
    thread = threading.Thread(target=run_detect_foo)
    thread.start()
...
def run_detect_foo():
    result = predict()
    print(result)
    # Update the text on the canvas from the main thread
    if interrupt_flag:
        canvas1.after(0, lambda: canvas1.itemconfigure(process_started_text, text="Processing Interrupted."))
    else:
        canvas1.after(0, lambda: canvas1.itemconfigure(process_started_text, text="Processing Completed."))
...
def download():
    ...
    if output_file:
        ...
        destination_path = filedialog.asksaveasfilename(initialdir=os.path.expanduser("~"), title="Save File As")
        ...
        if destination_path:
            ...
            destination_path_with_extension = destination_path + file_extension

            # Copy the file to the destination
            shutil.copy(output_file, destination_path_with_extension)
...
canvas1.create_text(420, 120, text="VEHICLE CLASSIFICATION, TRACKING AND COUNTING",
                                            fill="white", font=('Helvetica 18 bold'))


button1 = Button(root, text="Choose File",
                                            fg='black', font=("Helvetica", 15), command=upload)

button1_canvas = canvas1.create_window(100, 310, anchor="nw",
                                                                window=button1)

file_selected_text = canvas1.create_text(160, 360, text="No file Selected",
                                            fill="white", font=('Helvetica 13'))

button2 = Button(root, text="Start Processing",
                                            fg='black', font=("Helvetica", 15), command=process)
button2_canvas = canvas1.create_window(350, 310, anchor="nw",
                                                                window=button2)

process_started_text = canvas1.create_text(430, 430, text="",
                                            fill="white", font=('Helvetica 15 bold'))

button3 = Button(root, text="Download",
                                            fg='black', font=("Helvetica", 15), command=download)
button3_canvas = canvas1.create_window(600, 310, anchor="nw",
                                                                window=button3)

# Bind the key press event to the interrupt_process() function
# root.bind("<Key>", interrupt_process)
